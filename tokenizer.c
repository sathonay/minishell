...
/*   Updated: 2025/09/14 21:06:35 by alrey            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_token_stack	*new_token(char *start,
										enum e_token_type type)
{
	...
	if (token == NULL)
		return (NULL);
	token->start = start;
	token->end = start + get_token_length(type);
	token->type = type;
	token->next = NULL;
	return (token);
}
...
static int	extract_token(t_shell *shell, char **head)
{
	...
	if (to_token_type(*head) == NONE)
		return (0);
	token = new_token(*head, to_token_type(*head));
	if (!append_token(shell, token))
		return (2);
...
int	tokenize(t_shell *shell)
{
	...
	input = shell->input;
	token_code = 1;
	while (*input && token_code == 1)
		token_code = extract_token(shell, &input);
	if (token_code == 2)
		return (printf("NEM: failed to allocate token struct\n"), 0);
	if (*input != 0)
		printf("unclosed \" or '\n");
	return (*input == 0);
}
